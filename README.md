...
- Год 1:
  \[
  PV_1 = \frac{300}{(1+0,10)^1} \approx 272,73 \text{ монет}
  \]
- Год 2:
  \[
  PV_2 = \frac{400}{(1+0,10)^2} \approx 330,58 \text{ монет}
  \]
- Год 3:
  \[
  PV_3 = \frac{500}{(1+0,10)^3} \approx 375,66 \text{ монет}
  \]
- Год 4:
  \[
  PV_4 = \frac{600}{(1+0,10)^4} \approx 409,49 \text{ монет}
  \]
- Год 5:
  \[
  PV_5 = \frac{700}{(1+0,10)^5} \approx 434,58 \text{ монет}
  \]

Общая приведенная стоимость будущих денежных потоков:
\[
\text{Суммарный PV} = PV_1 + PV_2 + PV_3 + PV_4 + PV_5 \approx 272,73 + 330,58 + 375,66 + 409,49 + 434,58 \approx 1823,04 \text{ монет}
\]

Рассчитываем NPV:
\[
NPV = \text{Суммарный PV} - \text{Первоначальные инвестиции} = 1823,04 - 1000 = 823,04 \text{ монет}
\]

Поскольку NPV положительный (823,04 монет), проект считается выгодным.

### IRR (Внутренняя норма доходности)

IRR — это такая ставка дисконтирования, при которой NPV проекта равен нулю. Иными словами, это "магическая сила" вашей волшебной корзины, при которой приведенная стоимость будущих денежных потоков ровно равна вашим первоначальным инвестициям.

Чтобы найти IRR, нужно решить уравнение:
\[
1000 = \frac{300}{(1+IRR)^1} + \frac{400}{(1+IRR)^2} + \frac{500}{(1+IRR)^3} + \frac{600}{(1+IRR)^4} + \frac{700}{(1+IRR)^5}
\]

Это уравнение сложно решить аналитически, поэтому обычно используется метод проб и ошибок или финансовый калькулятор.

Предположим, что при IRR ≈ 15% приведенная стоимость будущих денежных потоков равна 1000 монет:

- Год 1:
  \[
  PV_1 = \frac{300}{(1+0,15)^1} \approx 260,87 \text{ монет}
  \]
- Год 2:
  \[
  PV_2 = \frac{400}{(1+0,15)^2} \approx 302,03 \text{ монет}
  \]
- Год 3:
  \[
  PV_3 = \frac{500}{(1+0,15)^3} \approx 329,20 \text{ монет}
  \]
- Год 4:
  \[
  PV_4 = \frac{600}{(1+0,15)^4} \approx 342,16 \text{ монет}
  \]
- Год 5:
  \[
  PV_5 = \frac{700}{(1+0,15)^5} \approx 347,20 \text{ монет}
  \]

Сумма приведенных стоимостей:
\[
\text{Суммарный PV при IRR 15%} \approx 260,87 + 302,03 + 329,20 + 342,16 + 347,20 \approx 1581,46 \text{ монет}
\]

Но мы видим, что сумма все еще больше 1000 монет, значит IRR должен быть выше. Продолжая этот процесс, мы найдем точное значение IRR, при котором NPV будет равен нулю.

### Связь между NPV и IRR

- Если IRR > стоимость денег (дисконтная ставка), то NPV > 0, и проект выгоден.
- Если IRR < стоимость денег, то NPV < 0, и проект невыгоден.
- Если IRR = стоимость денег, то NPV = 0.

В нашем примере:

- При стоимости денег 10% и IRR ≈ 15% проект выгоден, так как NPV положительный (823,04 монет).
- Если бы стоимость денег была 20%, а IRR оставался 15%, то NPV был бы отрицательным, и проект был бы невыгоден.
